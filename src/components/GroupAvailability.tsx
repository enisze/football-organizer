...
const aggregateTimeSlots = (slots: any[], duration: TimeSlotDuration, users: User[]) => {
    ...
    if (i + slotsPerGroup <= slots.length) {
      ...
      groupSlots.forEach((slot) => {
        slot.availableUsers.forEach((user: User) => {
          ...
          userAvailabilityCounts[user.id] = (userAvailabilityCounts[user.id] || 0) + 1
        })
      })
      ...
      const availableUsers = Array.from(allUsers)
        .filter((userId) => (userAvailabilityCounts[userId] || 0) >= minRequiredSlots)
...
export function GroupAvailabilityView({ date, users }: GroupAvailabilityViewProps) {
  const [duration, setDuration] = useState<TimeSlotDuration>("30min")

  const rawAvailabilityData = useMemo(() => generateMockAvailability(date, users), [date, users])
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-4">
        ...
        <Tabs value={duration} onValueChange={(value) => setDuration(value as TimeSlotDuration)} className="w-full">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {users.map((user) => (
          <div key={user.id} className="flex items-center gap-2 rounded-full border px-3 py-1 text-sm">
      ...
      >
        {availabilityData.map((slot, index) => {
          ...
          const percentage = users.length > 0 ? (availableCount / users.length) * 100 : 0
          ...
          return (
            ...
            >
              <div className="z-10 text-xs font-medium">{duration === "30min" ? slot.time : slot.timeRange}</div>
              ...
              <div
                ...
                style={{
                  height: `${percentage}%`,
                  backgroundColor: percentage === 0 ? "transparent" : `rgba(34, 197, 94, ${percentage / 100})`,
                }}
              />
